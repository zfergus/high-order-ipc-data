...
def attach_higher_order_nodes(V_P1, E_P1, F_P1, T_P1, order):
    if order == 1:
        return V_P1, T_P1
    ...
    edge_to_id = {tuple(e): i for i, e in enumerate(E_P1)}
    face_to_id = {tuple(f): i for i, f in enumerate(F_P1)}
    for ti, t in enumerate(T_P1):
        ...
        if order >= 4:
            ...
            T[ti, -1] = (V_P1.shape[0]
                         + nodes_per_edge * E_P1.shape[0]
                         + nodes_per_face * F_P1.shape[0]
                         + ti)
    assert(not (T == -1).any())
    V = np.vstack(V)
    assert(not (T >= V.shape[0]).any())
    return V, T
